...
export type SuspendAdminCouponInput = Readonly<{
  couponId: string;
  adminId: string;
  reason: string;
  ipAddress?: string | null;
...
export async function suspendAdminCoupon(
  input: SuspendAdminCouponInput,
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (
      ...
      throw new AdminCouponError({
        ...
        message:
          "Seul un code promo actif ou programmé peut être suspendu.",
        status: 409,
      });
    }
    ...
    await audit(tx, {
      ...
      action: "COUPON_SUSPENDED",
      previousStatus: coupon.status,
      newStatus: PromoCodeStatus.DISABLED,
      reason,
