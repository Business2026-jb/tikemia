...
export type RejectAdminMarketingCampaignInput = Readonly<{
  campaignId: string;
  adminId: string;
  reason: string;
...
export async function rejectAdminMarketingCampaign(
  input: RejectAdminMarketingCampaignInput,
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await createMarketingAuditLog(tx, {
      ...
      action: "MARKETING_CAMPAIGN_REJECTED",
      reason,
      metadata: {
