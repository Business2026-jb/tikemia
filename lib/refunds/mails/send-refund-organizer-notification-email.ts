...
export async function sendRefundOrganizerNotificationEmail({
  organizer,
  event,
  refund,
}: {
  organizer: RefundEmailOrganizer;
  event: RefundEmailEvent;
  refund: RefundEmailRequest;
}): Promise<{ sent: true; id: string | null }> {
  const url = appUrl("/organizer/refunds");
  const date = formatDate(refund.requestedAt);
  const rejectionBlock = "";

  const summary = `
    <p style="margin:0;color:#cbd5e1;line-height:1.7">Une nouvelle demande de remboursement nécessite votre vérification.</p>
  ...
  return sendRefundMail({
    to: organizer.email,
    subject: `Nouvelle demande de remboursement — ${refund.reference}`,
    html: layout("Nouvelle demande de remboursement", summary),
    text: [
      `Bonjour ${organizer.name},`,
      "",
      "Une nouvelle demande de remboursement nécessite votre vérification.",
