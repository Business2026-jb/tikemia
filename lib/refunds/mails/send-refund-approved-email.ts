...
export async function sendRefundApprovedEmail({
  ...
  const summary = `
    <p style="margin:0;color:#cbd5e1;line-height:1.7">Votre remboursement a été approuvé. Le traitement financier suit maintenant le circuit du prestataire de paiement.</p>
  ...
  return sendRefundMail({
    to: customer.email,
    subject: `Remboursement approuvé — ${refund.reference}`,
    html: layout("Remboursement approuvé", summary),
    text: [
      `Bonjour ${customer.name},`,
      "",
      "Votre remboursement a été approuvé. Le traitement financier suit maintenant le circuit du prestataire de paiement.",
