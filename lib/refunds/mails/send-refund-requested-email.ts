...
export async function sendRefundRequestedEmail({
  ...
  const summary = `
    <p style="margin:0;color:#cbd5e1;line-height:1.7">Votre demande a bien été enregistrée et sera examinée par l’organisateur.</p>
  ...
  return sendRefundMail({
    to: customer.email,
    subject: `Demande de remboursement reçue — ${refund.reference}`,
    html: layout("Demande de remboursement reçue", summary),
    text: [
      `Bonjour ${customer.name},`,
      "",
      "Votre demande a bien été enregistrée et sera examinée par l’organisateur.",
