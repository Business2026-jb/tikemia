...
export type ProcessWhatsAppDeliveriesOptions = {
  limit?: number;
...
export type ProcessedWhatsAppDeliveryItem = {
  orderId: string;
...
export type ProcessWhatsAppDeliveriesResult = {
  startedAt: string;
  finishedAt: string;
  ...
  items: ProcessedWhatsAppDeliveryItem[];
};
  ...
  const result =
    await prisma.deliveryLog.updateMany({
      ...
      data: {
        ...
        errorMessage:
          "Traitement WhatsApp interrompu avant sa finalisation.",
      },
    });
...
export async function processWhatsAppDeliveries(
  options:
    ProcessWhatsAppDeliveriesOptions =
    {},
): Promise<
  ProcessWhatsAppDeliveriesResult
> {
  const startedAt =
    options.now ??
    new Date();
  ...
  for (
    ...
    try {
      ...
      console.error(
        "[PROCESS_WHATSAPP_DELIVERY_ERROR]",
        getPaymentErrorLogContext(
          error,
        ),
      );
      ...
      if (
        ...
        await markDeliveryGroupFailed({
          ...
          (
            ...
            console.error(
              "[PROCESS_WHATSAPP_DELIVERY_FAILURE_PERSIST_ERROR]",
              getPaymentErrorLogContext(
                persistenceError,
              ),
            );
      ...
      items.push({
        ...
        errorMessage:
          paymentError.exposeMessage
            ? paymentError.message
            : "L’envoi WhatsApp des billets a échoué.",
      });
    }
  }
...
export async function processSingleOrderTicketWhatsApp({
  orderId,
  forceResend = false,
  logoPath,
}: {
...
> {
  ...
  const result =
    await processWhatsAppDeliveries({
      limit:
        1,
  ...
  if (
    ...
    throw new PaymentError({
      ...
      message:
        "Aucune livraison WhatsApp en attente n’a été trouvée pour cette commande.",
