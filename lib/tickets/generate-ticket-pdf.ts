 ...
 * Racine unique des assets publics Tikemia.

 *

 * Le chemin final des images locales est toujours

 * contrôlé pour rester strictement dans /public.
 ...
 * Transforme une URL locale /public/... ou un chemin relatif

 * en chemin absolu sécurisé à l'intérieur du dossier public.

 *

 * Aucun chemin ne peut remonter en dehors de /public.
...
function resolveLocalPublicImagePath(
   ...
   * Le chemin est normalisé et vérifié avant toute lecture.

   * La validation ci-dessous garantit que le fichier

   * reste obligatoirement dans /public.
...
async function loadOptionalLogo(
   ...
   * On garde la compatibilité avec un logoPath personnalisé.

   *

   * Si aucun chemin n'est transmis, /public/logo.png est utilisé.
  ...
  if (
     ...
     * Un logoPath absolu était déjà accepté historiquement.

     * On conserve donc ce comportement pour ne pas casser

     * un éventuel appel existant.
...
async function readEventImageBytes(
  ...
  try {
     ...
     * Lecture volontairement dynamique d'un fichier déjà

     * validé comme appartenant à /public.
...
}: GenerateTicketPdfOptions): Promise<GeneratedTicketPdf> {
...
}: GenerateOrderTicketPdfsOptions): Promise<GeneratedOrderTicketPdfs> {
