...
export type SendPromotionSuspendedEmailInput = Readonly<{
  to: string;
...
export async function sendPromotionSuspendedEmail(
  input: SendPromotionSuspendedEmailInput,
): Promise<MailResult> {
  ...
  return sendMail({
    to: input.to,
    subject: `Promotion suspendue — ${eventTitle}`,
    heading: `Bonjour ${organizerName}, votre promotion a été suspendue`,
    intro: "La mise en avant de votre événement est temporairement suspendue.",
    rows: [["Événement", eventTitle], ["Référence", promotionId]],
    notice: reason ? `Motif : ${reason}` : "La promotion ne sera plus affichée comme active pendant sa suspension.",
    noticeTone: "warning",
  });
}
