...
export type SendPromotionApprovedEmailInput = Readonly<{
  to: string;
...
export async function sendPromotionApprovedEmail(
  input: SendPromotionApprovedEmailInput,
): Promise<MailResult> {
  ...
  return sendMail({
    to: input.to,
    subject: `Promotion approuvée — ${eventTitle}`,
    heading: `Bonjour ${organizerName}, votre promotion est approuvée`,
    intro: "Votre demande de mise en avant a été validée par l’administration Tikemia.",
    rows: [
      ["Événement", eventTitle],
      ["Référence", promotionId],
      ["Début", formatDate(input.startsAt)],
      ["Fin", formatDate(input.endsAt)],
      ["Priorité", input.priority == null ? null : String(input.priority)],
    ],
    notice: "L’événement sera mis en avant pendant la période prévue, selon les règles d’affichage de Tikemia.",
    noticeTone: "success",
  });
}
