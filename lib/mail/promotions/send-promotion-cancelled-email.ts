...
export type SendPromotionCancelledEmailInput = Readonly<{
  to: string;
...
export async function sendPromotionCancelledEmail(
  input: SendPromotionCancelledEmailInput,
): Promise<MailResult> {
  ...
  return sendMail({
    to: input.to,
    subject: `Promotion annulée — ${eventTitle}`,
    heading: `Bonjour ${organizerName}, votre promotion a été annulée`,
    intro: "La campagne de mise en avant de votre événement a été annulée.",
    rows: [["Événement", eventTitle], ["Référence", promotionId]],
    notice: reason ? `Motif : ${reason}` : "L’événement ne bénéficiera plus de cette mise en avant.",
    noticeTone: "danger",
  });
}
