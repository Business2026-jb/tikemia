...
export type SendPromotionRejectedEmailInput = Readonly<{
  to: string;
...
export async function sendPromotionRejectedEmail(
  input: SendPromotionRejectedEmailInput,
): Promise<MailResult> {
  ...
  const reason = required(input.reason, "Le motif du refus");

  return sendMail({
    to: input.to,
    subject: `Promotion non approuvée — ${eventTitle}`,
    heading: `Bonjour ${organizerName}, votre promotion n’a pas été approuvée`,
    intro: "L’administration Tikemia a examiné votre demande de mise en avant.",
