...
export type SendPromotionPriorityUpdatedEmailInput = Readonly<{
  to: string;
  organizerName: string;
  eventTitle: string;
  promotionId: string;
  previousPriority?: number | string | null;
  newPriority: number | string;
}>;

export async function sendPromotionPriorityUpdatedEmail(
  input: SendPromotionPriorityUpdatedEmailInput,
): Promise<MailResult> {
  ...
  return sendMail({
    to: input.to,
    subject: `Priorité de promotion mise à jour — ${eventTitle}`,
    heading: `Bonjour ${organizerName}, la priorité de votre promotion a changé`,
    intro: "L’administration Tikemia a mis à jour le niveau de priorité de votre mise en avant.",
    rows: [
      ["Événement", eventTitle],
      ["Référence", promotionId],
      ["Ancienne priorité", input.previousPriority == null ? null : String(input.previousPriority)],
      ["Nouvelle priorité", String(input.newPriority)],
    ],
    notice: "Cette priorité est utilisée par Tikemia pour organiser l’affichage des événements promus.",
    noticeTone: "info",
  });
}
