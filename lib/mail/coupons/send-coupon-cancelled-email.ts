...
export type SendCouponCancelledEmailInput = Readonly<{
  to: string;
...
export type SendCouponCancelledEmailResult = Readonly<{
  sent: boolean;
  provider: "RESEND";
...
export async function sendCouponCancelledEmail(
  input: SendCouponCancelledEmailInput,
): Promise<SendCouponCancelledEmailResult> {
  ...
  const subject = `Code promo annulé — ${code}`;

  const text = [
    `Bonjour ${organizerName},`,
    "",
    "L’administration Tikemia a annulé et archivé votre code promo.",
    "Il ne peut désormais plus être utilisé pour de nouvelles commandes.",
    "",
    ...
    "",
    "Les commandes et utilisations déjà enregistrées restent conservées dans votre historique.",
    "",
      ...
      <div style="max-width:640px;margin:0 auto;">
        ...
        >
          ...
          <h1
            ...
            "
          >
            Bonjour ${escapeHtml(organizerName)}, votre code promo a été annulé
          </h1>
          ...
          <p
            ...
            "
          >
            L’administration Tikemia a annulé et archivé ce code promo.
            Il ne peut désormais plus être utilisé pour de nouvelles commandes.
          </p>
          ...
          <div
            ...
            "
          >
            Les commandes et utilisations déjà enregistrées restent conservées
            dans votre historique Tikemia.
          </div>
  ...
  if (result.error) {
    throw new Error(
      result.error.message ||
        "Resend n’a pas pu envoyer l’e-mail d’annulation du code promo.",
    );
  }
