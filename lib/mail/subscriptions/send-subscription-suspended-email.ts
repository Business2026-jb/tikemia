...
export type SendSubscriptionSuspendedEmailInput =
  Readonly<{
    to: string;
...
export type SendSubscriptionSuspendedEmailResult =
  Readonly<{
    sent: boolean;
    provider: "RESEND";
...
export async function sendSubscriptionSuspendedEmail(
  input: SendSubscriptionSuspendedEmailInput,
): Promise<SendSubscriptionSuspendedEmailResult> {
  const recipient =
    normalizeRequired(
      input.to,
  ...
  const subject =
    `Votre abonnement Tikemia a été suspendu — ${planName}`;

  const text = [
    `Bonjour ${organizerName},`,
    "",
    "Votre abonnement organisateur Tikemia a été suspendu.",
    `Plan : ${planName}`,
    `Référence : ${subscriptionId}`,
    `Date de suspension : ${formatDate(input.suspendedAt)}`,
    `Date de fin prévue : ${formatDate(input.endsAt)}`,
    `Motif : ${reason}`,
    "",
    "Les privilèges liés à ce plan sont temporairement indisponibles.",
    "",
...
<body style="margin:0;background:#f4f6f8;font-family:Arial,Helvetica,sans-serif;color:#111827;">
  <div style="max-width:680px;margin:0 auto;padding:24px;">
    ...
    <div style="background:#061015;padding:28px;border-radius:0 0 16px 16px;">
      ...
      <p style="margin:18px 0 6px;font-size:12px;font-weight:800;letter-spacing:1.5px;color:#fbbf24;">ABONNEMENT SUSPENDU</p>
      <h1 style="margin:0;font-size:28px;line-height:1.2;color:#ffffff;">Vos privilèges sont temporairement désactivés</h1>
      <p style="margin:16px 0 0;color:#cbd5e1;line-height:1.7;">Bonjour ${escapeHtml(organizerName)}, l’administration Tikemia a suspendu votre abonnement.</p>
    </div>
    ...
    <div style="background:#ffffff;padding:26px;border-radius:16px;margin-top:14px;border:1px solid #e5e7eb;">
      <table role="presentation" style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tr><td style="padding:10px 0;color:#6b7280;">Date de suspension</td><td style="padding:10px 0;text-align:right;font-weight:800;">${escapeHtml(formatDate(input.suspendedAt))}</td></tr>
        <tr><td style="padding:10px 0;color:#6b7280;">Fin prévue</td><td style="padding:10px 0;text-align:right;font-weight:800;">${escapeHtml(formatDate(input.endsAt))}</td></tr>
      </table>
      ...
      <div style="margin-top:20px;padding:16px;border-radius:12px;background:#fffbeb;border:1px solid #fde68a;color:#92400e;line-height:1.7;">
        <strong>Motif de suspension</strong><br />
        ${escapeHtml(reason)}
      </div>
    </div>
  ...
  if (result.error) {
    throw new Error(
      ...
      "Resend n’a pas pu envoyer l’e-mail de suspension de l’abonnement.",
    );
  }
