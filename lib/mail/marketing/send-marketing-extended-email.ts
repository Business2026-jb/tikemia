...
export type SendMarketingExtendedEmailInput = Readonly<{
  to: string;
...
export async function sendMarketingExtendedEmail(
  input: SendMarketingExtendedEmailInput,
): Promise<MarketingEmailResult> {
  ...
  const newEndsAt =
    input.newEndsAt instanceof Date
      ? input.newEndsAt
      : new Date(input.newEndsAt);

  if (Number.isNaN(newEndsAt.getTime())) {
    throw new Error("La nouvelle date de fin est invalide.");
  }

  const additionalDays =
    input.additionalDays != null &&
  ...
  return sendMarketingEmail({
    to: input.to,
    subject: `Campagne marketing prolongée — ${campaignName}`,
    heading: `Bonjour ${organizerName}, votre campagne a été prolongée`,
    introduction:
      "L’administration Tikemia a modifié la date de fin de votre campagne marketing.",
    rows: [
      ["Campagne", campaignName],
      ["Référence", campaignId],
      ["Événement", eventTitle],
      ["Ancienne fin", formatDate(input.previousEndsAt)],
      ["Nouvelle fin", formatDate(newEndsAt)],
      [
        "Durée ajoutée",
        additionalDays
    ...
    notice: reactivated
      ? "La campagne a également été réactivée et peut de nouveau être diffusée."
      : "La campagne conserve son statut actuel jusqu’à une éventuelle réactivation.",
    noticeTone: reactivated ? "success" : "info",
    actionLabel: "Gérer mes campagnes",
