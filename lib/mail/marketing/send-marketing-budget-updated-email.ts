...
export type SendMarketingBudgetUpdatedEmailInput = Readonly<{
  to: string;
  organizerName: string;
  campaignId: string;
  campaignName: string;
  eventTitle: string;
  previousBudget?: string | number | null;
  newBudget?: string | number | null;
  currency: string;
  reason?: string | null;
}>;

export async function sendMarketingBudgetUpdatedEmail(
  input: SendMarketingBudgetUpdatedEmailInput,
): Promise<MarketingEmailResult> {
  ...
  return sendMarketingEmail({
    to: input.to,
    subject: `Budget marketing mis à jour — ${campaignName}`,
    heading: `Bonjour ${organizerName}, le budget de votre campagne a été modifié`,
    introduction:
      "L’administration Tikemia a mis à jour le budget enregistré pour votre campagne marketing.",
    rows: [
      ...
      [
        ...
        input.previousBudget != null
          ? formatAmount(input.previousBudget, currency)
          : "Non défini",
      ],
      [
        "Nouveau budget",
        input.newBudget != null
          ? formatAmount(input.newBudget, currency)
          : "Non défini",
      ],
    ],
    notice: normalizeOptional(input.reason)
      ? `Motif : ${normalizeOptional(input.reason)}`
      : "Le nouveau budget est désormais appliqué à cette campagne.",
    noticeTone: "info",
