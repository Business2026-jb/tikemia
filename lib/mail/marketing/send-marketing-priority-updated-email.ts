...
export type SendMarketingPriorityUpdatedEmailInput = Readonly<{
  to: string;
...
export async function sendMarketingPriorityUpdatedEmail(
  input: SendMarketingPriorityUpdatedEmailInput,
): Promise<MarketingEmailResult> {
  ...
  const newPriority = normalizeRequired(
    input.newPriority,
  ...
  return sendMarketingEmail({
    to: input.to,
    subject: `Priorité marketing mise à jour — ${campaignName}`,
    heading: `Bonjour ${organizerName}, la priorité de votre campagne a été modifiée`,
    introduction:
      "L’administration Tikemia a ajusté le niveau de priorité de votre campagne marketing.",
    rows: [
      ...
      [
        "Ancienne priorité",
        input.previousPriority
          ? formatPriority(input.previousPriority)
          : "Normale",
      ],
      ["Nouvelle priorité", formatPriority(newPriority)],
    ],
    notice: normalizeOptional(input.reason)
      ? `Motif : ${normalizeOptional(input.reason)}`
      : "La nouvelle priorité sera prise en compte dans l’ordre de diffusion des campagnes.",
    noticeTone:
      newPriority === "URGENT" || newPriority === "HIGH"
        ? "warning"
        : "info",
    actionLabel: "Consulter ma campagne",
