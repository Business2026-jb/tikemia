...
export type SendMarketingApprovedEmailInput = Readonly<{
  to: string;
...
export async function sendMarketingApprovedEmail(
  input: SendMarketingApprovedEmailInput,
): Promise<MarketingEmailResult> {
  ...
  const currency = normalizeOptional(input.currency) ?? "XOF";

  return sendMarketingEmail({
    to: input.to,
    subject: `Campagne marketing approuvée — ${campaignName}`,
    heading: `Bonjour ${organizerName}, votre campagne a été approuvée`,
    introduction:
      "L’administration Tikemia a validé votre campagne marketing. Elle sera diffusée selon la période et les paramètres approuvés.",
    rows: [
      ["Campagne", campaignName],
      ["Référence", campaignId],
      ["Événement", eventTitle],
      ["Canal", normalizeOptional(input.channel)],
      ["Budget", input.budget != null ? formatAmount(input.budget, currency) : null],
      ["Début", formatDate(input.startsAt)],
      ["Fin", formatDate(input.endsAt)],
    ],
    notice:
      "La campagne sera visible uniquement pendant sa période active et tant que l’événement respecte les conditions de publication Tikemia.",
    noticeTone: "success",
