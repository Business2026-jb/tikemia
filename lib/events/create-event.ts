...
/*
 * Ces champs sont conservés dans les schémas afin de rester compatibles
 * avec les anciennes versions du formulaire et les types déjà utilisés.
 *
 * Le service les remplace systématiquement par null avant validation.
 */
const optionalSaleDateSchema =
...
const ticketTypeSchema = z.object({
  ...
  price: z.union([
    z
      .number()
      .finite(
        "Le prix du billet n’est pas valide.",
      )
      .nonnegative(
  ...
  /*
   * Compatibilité avec l’ancien contrat.
   * Ces valeurs seront toujours neutralisées à null.
   */
  saleStartsAt:
    optionalSaleDateSchema,
...
const eventImageSchema = z.object({
  path: z
    ...
    .max(
      1_000,
      "Le chemin de l’image est trop long.",
    ),
  ...
  publicUrl: z
    ...
    .trim()
    .url(
      "L’adresse publique de l’image n’est pas valide.",
    )
    .max(
      2_000,
  ...
  position: z
    .number()
    .int(
      "La position de l’image doit être un entier.",
    )
    .min(
      0,
...
const createEventSchema = z.object({
  ...
  categoryId: z
    ...
    .min(
      1,
      "La catégorie de l’événement est obligatoire.",
    ),
  ...
  coverImage: z
    ...
    .trim()
    .url(
      "L’adresse de l’image principale n’est pas valide.",
    )
    .max(
      2_000,
  ...
  address: z
    ...
    .min(
      3,
      "L’adresse de l’événement est obligatoire.",
    )
    .max(
      300,
   ...
   * Le service remplace ces deux valeurs par null.
   */
  salesStartAt:
    optionalSaleDateSchema,
...
/**
 * Neutralise les anciennes périodes de vente.
 *
 * Cette normalisation est effectuée dans le service, même si la route
 * l’a déjà faite. Cela protège aussi les futurs appels internes qui
 * appelleraient createEvent() sans passer par la route HTTP actuelle.
 */
function normalizeSimplifiedSalesInput(
...
function validateEventDates({
  ...
  if (
    ...
    throw new CreateEventError({
      code:
        "INVALID_EVENT_START_DATE",

      message:
        "La date de début de l’événement n’est pas valide.",

      fields: {
        startsAt: [
          "Renseignez une date et une heure de début valides.",
        ],
      },
    });
  }
  ...
  if (
    ...
    throw new CreateEventError({
      ...
      fields: {
        endsAt: [
          "Renseignez une date et une heure de fin valides.",
        ],
      },
    });
  }
    ...
    (
      ...
      if (
        ...
        throw new CreateEventError({
          ...
          message:
            `La limite par commande du billet « ${ticketType.name} » dépasse sa quantité disponible.`,
...
async function generateUniqueEventSlug(
  ...
  const exactSlugExists =
    await transaction.event.findUnique({
      where: {
        slug:
          baseSlug,
      },
...
export async function createEvent(
  ...
  let projection:
    ReturnType<
      typeof calculateEventRevenueProjection
    >;
      ...
      async (
        ...
        if (!coverImage) {
          throw new CreateEventError({
            ...
            message:
              "Aucune image principale valide n’a été trouvée.",

            fields: {
              images: [
                "Ajoutez une image principale à l’événement.",
              ],
            },
          });
        }
        ...
        const createdEvent =
          await transaction.event.create({
            data: {
              ...
              /*
               * Les ventes n’ont plus de période configurée
                ...
                create:
                  data.ticketTypes.map(
                    (
                      ...
                      /*
                       * Les billets sont disponibles selon
                       * leur statut actif et les règles
                       * générales de la billetterie.
                       */
                      saleStartsAt:
                        null,
        ...
        return {
          event: {
            ...
            salesStartAt:
              createdEvent.salesStartAt
                ?.toISOString() ??
              null,

            salesEndAt:
              createdEvent.salesEndAt
                ?.toISOString() ??
              null,
