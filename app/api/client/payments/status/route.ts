...
function getPaymentLookup(
  ...
  const rawPaymentId = normalizeText(
    url.searchParams.get("paymentId"),
  );
  const rawOrderId = normalizeText(
    url.searchParams.get("orderId"),
  );

  if (!rawPaymentId && !rawOrderId) {
    throw new PaymentValidationError({
  ...
  const rawValue =
    rawPaymentId || rawOrderId;

  const parsedValue =
    identifierSchema.safeParse(rawValue);

  if (!parsedValue.success) {
    throw new PaymentValidationError({
      code: "PAYMENT_INVALID_REQUEST",
      message:
        parsedValue.error.issues[0]
          ?.message ??
        "L’identifiant du paiement est invalide.",
      status: 400,
    });
  ...
  try {
    ...
    return jsonResponse({
      ...
      tickets: {
        ...
        expected:
          expectedTicketsCount,
        issued: issuedTicketsCount,
        valid: validTicketsCount,
      },
      redirectTo,
    });
  } catch (error) {
