  ...
  try {
    ...
    if (!event) {
      throw new ScannerError({
        ...
        message:
          "Cet événement est introuvable ou n’est plus disponible.",
    ...
    return jsonResponse({
      ...
      message:
        "Événement scanner chargé.",

      event,
    });
  } catch (error) {
    console.error(
      "[SCANNER_EVENT_GET_ERROR]",
      error,
    );
