...
import { suspendAdminMarketingCampaign } from "@/lib/admin/marketing/suspend-admin-marketing-campaign";
import { sendMarketingSuspendedEmail } from "@/lib/mail/marketing/send-marketing-suspended-email";
...
export async function POST(
  ...
  try {
    ...
    const result = await suspendAdminMarketingCampaign({
    ...
    try {
      await sendMarketingSuspendedEmail({
      ...
      console.error("[ADMIN_MARKETING_SUSPEND_EMAIL_ERROR]", emailError);
    }

    return jsonResponse({
      success: true,
      message: "La campagne marketing a été suspendue.",
      data: {
        ...result,
        emailSent,
      },
    ...
    console.error("[ADMIN_MARKETING_SUSPEND_ERROR]", error);
