...
import { rejectAdminMarketingCampaign } from "@/lib/admin/marketing/reject-admin-marketing-campaign";
import { sendMarketingRejectedEmail } from "@/lib/mail/marketing/send-marketing-rejected-email";
...
export async function POST(
  ...
  try {
      ...
      return jsonResponse(
        {
          success: false,
          error: {
            code: "ADMIN_MARKETING_REASON_REQUIRED",
            message:
    ...
    const result = await rejectAdminMarketingCampaign({
    ...
    try {
      await sendMarketingRejectedEmail({
      ...
      console.error("[ADMIN_MARKETING_REJECT_EMAIL_ERROR]", emailError);
    }

    return jsonResponse({
      success: true,
      message: "La campagne marketing a été refusée et archivée.",
      data: {
        ...result,
        emailSent,
      },
    ...
    console.error("[ADMIN_MARKETING_REJECT_ERROR]", error);
