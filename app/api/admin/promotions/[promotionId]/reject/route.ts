...
} from "@/lib/admin/promotions/reject-admin-promotion";
import {
  requireAdmin,
} from "@/lib/admin/require-admin";
import {
  sendPromotionRejectedEmail,
} from "@/lib/mail/promotions/send-promotion-rejected-email";
  ...
  try {
      ...
      return jsonResponse(
        {
          ...
          error: {
            ...
            message:
              ...
              "Le motif du refus est invalide.",
            details: {
              fields:
    ...
    const emailSent =
      await sendEmailSafely(
        ...
        "[PROMOTION_REJECTED_EMAIL_ERROR]",
      );

    return jsonResponse({
      success: true,
      message: emailSent
        ? "La promotion a été refusée et l’organisateur a été notifié."
        : "La promotion a été refusée, mais l’e-mail n’a pas pu être envoyé.",
      data: {
        ...result,
        emailSent,
      },
