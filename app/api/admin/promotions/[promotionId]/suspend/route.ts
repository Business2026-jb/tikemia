...
import {
  suspendAdminPromotion,
} from "@/lib/admin/promotions/suspend-admin-promotion";
import {
  requireAdmin,
} from "@/lib/admin/require-admin";
import {
  sendPromotionSuspendedEmail,
} from "@/lib/mail/promotions/send-promotion-suspended-email";
  ...
  try {
    ...
    const emailSent =
      await sendEmailSafely(
        ...
        "[PROMOTION_SUSPENDED_EMAIL_ERROR]",
      );

    return jsonResponse({
      success: true,
      message: emailSent
        ? "La promotion a été suspendue et l’organisateur a été notifié."
        : "La promotion a été suspendue, mais l’e-mail n’a pas pu être envoyé.",
      data: {
        ...result,
        emailSent,
      },
