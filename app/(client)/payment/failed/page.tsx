...
function FailedFallback() {
  return (
    <div className="flex min-h-[72vh] w-full items-center justify-center px-4 py-10">
      <div className="h-10 w-10 animate-spin rounded-full border-4 border-white/[0.08] border-t-red-400" />
    </div>
  );
}
...
function PaymentFailedContent() {
        ...
        async () => {
          try {
            ...
            if (
              ...
              setFailureMessage(
                "La réservation a expiré avant la confirmation du paiement.",
              );
            } else if (
              failureCode
            ) {
              setFailureMessage(
                "Le paiement a été refusé par le prestataire.",
              );
            }
          } catch {
            return;
  ...
  return (
    <div className="relative flex min-h-[76vh] w-full items-center justify-center overflow-hidden px-4 py-10 sm:px-6 lg:px-8">
      ...
      >
        <div className="absolute left-1/2 top-[44%] h-[480px] w-[480px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-red-500/[0.06] blur-[140px]" />
      </div>
      ...
      <section className="relative w-full max-w-2xl overflow-hidden rounded-[34px] border border-white/[0.09] bg-[#071015]/96 p-6 text-center shadow-[0_30px_100px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:p-10">
        <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-[30px] border border-red-400/20 bg-red-400/[0.08] text-red-400">
          <AlertTriangle
            aria-hidden="true"
            className="h-12 w-12"
          />
        ...
        <p className="mt-7 text-xs font-black uppercase tracking-[0.24em] text-red-400">
          Paiement échoué
        </p>
        ...
        <div className="mt-7 flex items-center justify-center gap-2 text-xs leading-5 text-neutral-600">
          ...
          Aucun billet n’est généré sans paiement confirmé.
        </div>
      </section>
    </div>
  );
