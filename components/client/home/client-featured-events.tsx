    ...
    >
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-orange-500/20 bg-orange-500/[0.08] text-orange-300">
              <Flame
                aria-hidden="true"
                className="h-4 w-4"
              />
        ...
        <Link
          href={viewAllHref}
          className="group inline-flex h-10 shrink-0 items-center justify-center gap-2 self-start rounded-xl border border-lime-500/20 bg-lime-500/[0.06] px-4 text-xs font-black text-lime-300 transition hover:border-lime-500/30 hover:bg-lime-500/[0.1] sm:self-auto"
        >
          {viewAllLabel}
      ...
      {events.length > 0 ? (
        <div className="relative mt-5">
          ...
          <div className="-mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:-mx-5 sm:gap-4 sm:px-5 lg:mx-0 lg:grid lg:grid-cols-3 lg:overflow-visible lg:px-0 lg:pb-0 xl:grid-cols-5">
            {events.map(
              (
                event,
                index,
          ...
          <div className="mt-3 flex items-center justify-between gap-3 lg:hidden">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.06] bg-white/[0.02] px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.08em] text-neutral-600">
              <Sparkles className="h-3 w-3" />
              {events.length} sélection
              {events.length > 1
                ? "s"
                : ""}
            </span>
        ...
        <div className="mt-5 rounded-3xl border border-dashed border-white/[0.09] bg-white/[0.018] px-5 py-10 text-center">
          ...
          <Link
            href="/events"
            className="mt-5 inline-flex h-10 items-center justify-center rounded-xl border border-white/[0.09] bg-white/[0.025] px-4 text-xs font-black text-neutral-300 transition hover:border-white/[0.15] hover:bg-white/[0.05] hover:text-white"
          >
