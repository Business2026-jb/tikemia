...
export type PayoutExportFilters = {
  search: string;
  status: string;
...
export default function ExportPayoutsButton({
  filters,
}: {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const response =
        await fetch(
          `/api/admin/payouts/export?${params.toString()}`,
          {
            method: "GET",
            cache: "no-store",
          },
      ...
      const fileName =
        match?.[1] ||
        `rapport-retraits-tikemia-${Date.now()}.pdf`;
