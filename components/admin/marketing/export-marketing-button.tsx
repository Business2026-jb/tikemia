...
export type MarketingExportFilters = Readonly<{
  search: string;
  status: string;
  channel: string;
  organizerId: string;
...
export default function ExportMarketingButton({
  filters,
}: {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/marketing/export?${params.toString()}`,
        { cache: "no-store" },
      );
      ...
      const fileName =
        match?.[1] ||
        `rapport-marketing-tikemia-${Date.now()}.pdf`;
