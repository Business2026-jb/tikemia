...
export default function ExtendMarketingDialog({
  campaign,
  open,
  onClose,
  onSuccess,
  ...
  const [reactivateIfCompleted, setReactivateIfCompleted] = useState(true);
  ...
  const canSubmit =
    Number.isInteger(days) && days >= 1 && days <= 3650 && !submitting;
        ...
        throw new Error(
          typeof payload.error === "string"
            ...
            : payload.error?.message ||
                "Impossible de prolonger la campagne.",
        );
      }

      onSuccess(payload.message || "La campagne a été prolongée.");
      onClose();
    } catch (caught) {
      setError(
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-[24px] border border-amber-400/15 bg-[#070b0e] p-5 shadow-2xl sm:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-neutral-500">
          La nouvelle date ne pourra pas dépasser la fin de l’événement.
        </p>
        ...
        <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-white/[0.07] bg-white/[0.02] p-4">
          <input
            type="checkbox"
            checked={reactivateIfCompleted}
            onChange={(event) =>
              setReactivateIfCompleted(event.target.checked)
            }
            className="mt-1"
          />
          <span className="text-sm leading-6 text-neutral-400">
            Réactiver automatiquement si la campagne est terminée ou suspendue.
          </span>
        </label>
